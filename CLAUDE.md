...
Five independent projects share this repo. **Load only the docs for the project you're touching.**
...
| Breachpoint (Three.js AAA-tier FPS) | `fps/` | `fps/CLAUDE.md` then `fps/handoff.md` | `cd fps && npm run dev` · `npm test` |
...
- Root `handoff.md`/`plan.md`/`src/*.rs` belong to **Wii Kart only**; `carnival/*.md` to carnival only; `fps/*.md` + `fps/src/` to Breachpoint only. Never mix them.
